...
import { GetUserProgressionParams, GetUserAchievementsParams } from "@workspace/api-zod";
...
router.get("/users/:id/achievements", async (req, res): Promise<void> => {
  const params = GetUserAchievementsParams.safeParse(req.params);
  if (!params.success) {
...
router.get("/users/:id/progression", async (req, res): Promise<void> => {
  const params = GetUserProgressionParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
  }
